<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-100 leading-tight">
            {{ __('Ubah Pengumuman :') }} <span class="text-blue-200">{{ substr($announcement->title, 0, 10) }}</span>
        </h2>
    </x-slot>

    <!-- Section Ubah Pengumuman -->
    <div class="py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg p-8">
                <!-- Form Ubah Pengumuman -->
                <form action="{{ route('pengumuman.update', $announcement->slug) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <!-- Judul Pengumuman -->
                    <div class="mb-4">
                        <label for="title" class="block text-md font-medium text-gray-700">Judul Pengumuman</label>
                        <input type="text" name="title" id="title" required
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:border focus:border-[#141652]"
                            placeholder="Masukkan judul pengumuman"
                            value="{{ $announcement->title = old('title') ?? $announcement->title }}">
                    ...
                    <!-- Status Pengumuman -->
                    <div class="mb-4">
                        <label for="status" class="block text-md font-medium text-gray-700">Status Pengumuman</label>
                        <select name="status" id="status" required
                            ...
                            <option value="Terbit"
                                {{ (old('status') ?? $announcement->status) === 'Terbit' ? 'selected' : '' }}>Terbit
                            </option>
                            <option value="Arsip"
                                {{ (old('status') ?? $announcement->status) === 'Arsip' ? 'selected' : '' }}>
                                Arsip</option>
                        </select>
                    ...
                    <!-- Gambar Pengumuman -->
                    <div x-data="{ imagePreview: '{{ $announcement->image ? asset('storage/' . $announcement->image) : null }}' }" class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input id="body" type="hidden" name="body"
                            value="{{ $announcement->body = old('body') ?? $announcement->body }}">
                        <trix-editor
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:border focus:border-[#141652]"
